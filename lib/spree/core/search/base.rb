    ...
    module Search
      class Base
        ...
        def retrieve_products
          ...
          @price = @products.order("spree_prices.amount DESC")
          @max_price = !@price.empty? ? @price.last.price.to_s : "50000"
          @min_price = !@price.empty? ? @price.first.price.to_s : "0"
          if @properties[:search] && @properties[:search][:price_range_any]!="" && !@properties[:search][:price_range_any].empty?
            @products = @products.where("spree_prices.amount between #{@properties[:search][:price_range_any][0].split("-")[0]} and #{@properties[:search][:price_range_any][0].split("-")[1]}")
          end
          @products = @products.page(curr_page).per(per_page)
          return @products,@max_price,@min_price
        end
